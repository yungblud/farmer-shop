...
const render = async (ctx) => {
  ...
  routes.forEach(
    route => {
      ...
      if(url === '/') {
        const PostActions = bindActionCreators(postActions, store.dispatch);
        const CategoryActions = bindActionCreators(categoryActions, store.dispatch);
        const config = {
            headers: { Pragma: 'no-cache'}
        };
        const promise1 = PostActions.getPostItemList(config);
        const promise2 = CategoryActions.getAllCategories();
